...
def get_parameters(support_labels, shot_features, gamma_override=None, lambda_override=None):
    few_shot = shot_features is not None
    if few_shot:
        lambda_value = lambda_override if lambda_override is not None else 0.5
        if gamma_override is not None:
            gamma_list = [gamma_override] if isinstance(gamma_override, (int, float)) else gamma_override
        else:
        ...
        lambda_value = lambda_override if lambda_override is not None else 1
        gamma_list = [0]
        support_labels = None
