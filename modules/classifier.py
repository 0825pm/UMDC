...
class EchoClassfierF_TR(EchoClassfierF):
    ...
    def init_weight(self, cache_keys, cache_vals):
        ...
        self.float()  # Ensure consistent dtype after mixed-precision training
        with torch.no_grad():
    ...
    def forward(self, x):
        # During MVREC train_compact, TaskRes not yet initialized
        if not hasattr(self, 'taskres_U'):
            return EchoClassfierF.forward(self, x)
